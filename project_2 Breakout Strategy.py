...
def get_long_short(close, lookback_high, lookback_low):
    ...
    """
    #TODO: Implement function
    new_df = close.copy(); new_df.loc[:,:] = 0.0

    short = (lookback_low > close).astype(int)*-1
    
    long = (lookback_high < close).astype(int)
    
    new_df = short + long + new_df
...
def filter_signals(signal, lookahead_days):
    ...
    for col in new_df.columns:
        ...
        clean_signals = [0]*lookahead_days 

        # create clean signals 
        clean_signals.extend(list(new_df[col].values))
        
        # clean signals
        clean_signals_series = pd.DataFrame(clean_signals, columns = ['values'])

        for value in clean_signals_series[lookahead_days:].itertuples():
            if value.values == 0:
                pass
            elif value.values == 1:
                check_df = clean_signals_series.iloc[value.Index-lookahead_days:value.Index,]['values']
                if value.values in check_df.values:
                    clean_signals_series.loc[value.Index,'values'] = 0
            elif value.values == -1:
                check_df = clean_signals_series.iloc[value.Index-lookahead_days:value.Index,]['values']
                if value.values in check_df.values:
                    clean_signals_series.loc[value.Index,'values'] = 0
        new_df[col] = clean_signals_series[lookahead_days:].values.squeeze()
...
def get_lookahead_prices(close, lookahead_days):
    ...
    """
    #TODO: Implement function
    new_close = close.shift(-lookahead_days)
    return new_close
...
def get_return_lookahead(close, lookahead_prices):
    ...
    """
    #TODO: Implement function
    new_log_return = np.log(lookahead_prices) - np.log(close)
    
    return new_log_return
...
# The longer the investment horizon applied, the more positive signal returns were able to capture. As we can see from the 20-day signal return distribution, the shape of distribution is positively skewed, which is favorable for investors
...
def calculate_kstest(long_short_signal_returns):
    ...
    #TODO: Implement function
    for i in long_short_signal_returns.groupby('ticker'):
        ticker.append(i[0])
        # normal args should have the entire portfolio mean and std(n - 0)
        normal_args = (np.mean(long_short_signal_returns.signal_return), 
                       np.std(long_short_signal_returns.signal_return, ddof = 0)
                      )
        ks_values, p_values = kstest(i[1].signal_return,'norm',normal_args,)
        p_val.append(p_values)
        ks.append(ks_values)
    
    ks_values = pd.Series(ks, index = ticker)
    p_val = pd.Series(p_val, index = ticker)
    
    return ks_values,p_val
...
def find_outliers(ks_values, p_values, ks_threshold, pvalue_threshold=0.05):
    ...
    set1 = set(ks_values[ks_values > ks_threshold].index)
    
    set2 = set(p_values[p_values < pvalue_threshold].index)
