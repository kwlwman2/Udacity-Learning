...
def generate_dollar_volume_weights(close, volume):
    ...
    sum_dollar_weights = dollar_volume.sum(axis = 1)
    
    dollar_weights = dollar_volume.div(sum_dollar_weights,axis = 0)
...
def calculate_dividend_weights(dividends):
    ...
    div_cumsum = dividends.cumsum(axis = 0)
    
    date_div_cumsum = div_cumsum.sum(axis =1)
    
    dividend_weights = div_cumsum.div(date_div_cumsum,axis = 0)
...
def calculate_cumulative_returns(returns):
    ...
    """
    #TODO: Implement function
    cum_returns = (returns.sum(axis = 1)+1).cumprod()
...
def tracking_error(benchmark_returns_by_date, etf_returns_by_date):
    ...
    # TODO: Implement function
    sample_std = np.std(etf_returns_by_date - benchmark_returns_by_date,ddof = 1)

    TE = np.sqrt(252)*sample_std
...
def get_optimal_weights(covariance_returns, index_weights, scale=2.0):
    ...
    #TODO: Implement function
    # define how many variables we need to have
    m = covariance_returns.shape[0]
    x = cvx.Variable(m)
    
    # define objective function = x_T * Px
    portfolio_variance = cvx.quad_form(x, covariance_returns)
    distance_to_index = cvx.norm(x-index_weights)
    objective_function = portfolio_variance + distance_to_index * scale
    # define constraints
    constraints = [x >= 0, sum(x)==1]
    
    # set up the problem
    Obj = cvx.Minimize(objective_function)
    
    results = cvx.Problem(Obj,constraints).solve()
...
def rebalance_portfolio(returns, index_weights, shift_size, chunk_size):
    ...
    # Define the list of array for storing optimal weights
    weights = []
    ...
    for i in range(returns.shape[0]):
        ...
        # the first data point
            ...
            known = returns.iloc[:i,:].fillna(0)           
            known_index = index_weights.loc[known.index,:].iloc[-1]
            counter = i + shift_size 
            
            # implement optimization
            covariance_returns = get_covariance_returns(known)
            X = get_optimal_weights(covariance_returns, known_index, scale=2.0)
            weights.append(X)
        
        elif i > chunk_size and i == counter:
            known = returns.iloc[i-chunk_size:i,:].fillna(0)
            known_index = index_weights.loc[known.index,:].iloc[-1]
            counter = i + shift_size            
...
def get_portfolio_turnover(all_rebalance_weights, shift_size, rebalance_count, n_trading_days_in_year=252):
    ...
    SumTotalTurnover = df.diff().apply(np.abs).sum().sum()
